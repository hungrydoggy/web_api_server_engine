...
module.exports = {
  makeCrudApiDoc,
  makeApiDoc, 
  makeModelParamsObjForWrite,
  makeModelParamsObjForRead,
};



function makeModelParamsObjForWrite (Table, reviseItem=(item)=>{}) {
  const item = { comment: `Model:${Table.getTableName()}`, value: {} };
...
function makeModelParamsObjForRead (Table, reviseItem=(item)=>{}) {
  const item = { comment: `Model:${Table.getTableName()}`, value: {} };
...
function makeCrudApiDoc (
  ...
  if (permission_create !== null) {
    const doc = {
      ...
      request: {
        params: (()=>{
          const item = makeModelParamsObjForWrite(Table);
          item.comment = 'params for creating';
          return item;
        })(),
      },
      response: _makeCrudResponseObj({
        ...
        value: [
          (()=>{
            const item = makeModelParamsObjForRead(Table);
            item.comment = 'Model:' + Table.getTableName();
            return item;
          })(),
        ],
      }),
  ...
  if (permission_update !== null) {
    const doc = {
      ...
      request: {
        params: (()=>{
          const item = makeModelParamsObjForWrite(Table);
          item.comment = 'params for updating';
          return item;
        })(),
        options: (()=>{
          const item = makeModelParamsObjForRead(Table);
...
function makeApiDoc (path, method, permission, doc, preSave=(doc)=>{}) {
  ...
  // save to file
  const filepath = `${config.api_doc_root_path}${path}/${method}.json`.replace(/\/:/g, '%');
