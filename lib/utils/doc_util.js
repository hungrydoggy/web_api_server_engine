...
async function makeCrudApiDoc(
  ...
  if (permission_create !== null) {
    ...
    await makeApiDoc(path, 'post', permission_create, doc, preSave);
  }
  ...
  if (permission_read !== null) {
    ...
    await makeApiDoc(path, 'get', permission_read, doc, preSave);
    await makeApiDoc(path+'%id', 'get', permission_read, doc, preSave);
  }
  ...
  if (permission_update !== null) {
    ...
    await makeApiDoc(path+'%id', 'put', permission_update, doc, preSave);
  }
  ...
  if (permission_delete !== null) {
    ...
    await makeApiDoc(path+'%id', 'delete', permission_delete, doc, preSave);
  }
}
 ...
 *}
 */
async function makeApiDoc (path, method, permission, doc, preSave=(doc)=>{}) {
  if (!config.api_doc_root_path)
    return;
  ...
  await fs.promises.mkdir(node_path.dirname(filepath), {recursive: true});
