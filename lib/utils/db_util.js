...
module.exports = {
  ...
  getFieldPermissions,
  hasFieldPermission (table_name, internal_field, permissions, perm_ends=['/read']/* or '/write'*/) {
    ...
    for (const fp of field_permissions) {
      for (const p of permissions) {
        ...
        if (end_str === '/' || perm_ends.indexOf(end_str) >= 0)
          return true;
      }
    }
...
function linkByManyToMany (model, model_map) {
  const m2m_fields = [];
...
function _linkHiddenField (model, model_map, key, obj) {
...
function _getOptionStrFromComment (comment) {
