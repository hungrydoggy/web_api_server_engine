...
module.exports = {
  ...
  async commonPost (
    ...
    if (decoded && decoded.options_from_permission) {
      await _fixParamsWithQuery(Table, params, decoded.options_from_permission);
    }
  ...
  async commonGet (
    ...
    for (const itm of items)
      _filterResultWithOriginalOptions(original_opt, itm);
  ...
  async commonGetById (
    ...
    _filterResultWithOriginalOptions(original_opt, item);
  ...
  async loginTemplate (
    ...
    const token = jwt_util.createAccessToken(token_params, life_time_sec);
    return {
      statusCode: 200,
      body: {
        message: 'ok',
...
async function _fixParamsWithQuery (Table, params, query, key_must_be=false, ignore_id=false) {
  // check with where
...
function _filterResultWithOriginalOptions (options, result) {
  ...
  if (options.include) {
    ...
    for (const inc of options.include) {
      ...
      const ass = inc.association;
      const result_ass = result[ass];
      if (result_ass) {
        if (Array.isArray(result_ass)) {
          for (const ra of result_ass)
            _filterResultWithOriginalOptions(inc, ra);
        }else
          _filterResultWithOriginalOptions(inc, result_ass);
      }
    }
  }
  ...
  for (const key in result) {
    if (key[0] === '*' && (!ass_inc_map || !(key in ass_inc_map))) {
      delete result[key];
    }else if (att_check_map && !(key in att_check_map)) {
      delete result[key];
    }
  }
}
