...
async function makeCrudApiDoc(
  ...
  if (permission_create !== null) {
    ...
    await makeApiDoc(path, 'post', doc, preSave);
  }
  ...
  if (permission_read !== null) {
    ...
    await makeApiDoc(path, 'get', doc, preSave);
    await makeApiDoc(path+'%id', 'get', doc, preSave);
  }
  ...
  if (permission_update !== null) {
    ...
    await makeApiDoc(path+'%id', 'put', doc, preSave);
  }
  ...
  if (permission_delete !== null) {
    ...
    await makeApiDoc(path+'%id', 'delete', doc, preSave);
  }
}
 ...
 *}
 */
async function makeApiDoc (path, method, doc, preSave=(doc)=>{}) {
  if (!config.api_doc_root_path)
    return;
  ...
  doc.path   = path_head + ((path_params && path_params.length <= 0)? '': '/'+path_params.map((p)=>`<${p}>`).join('/'));
  doc.method = method;
