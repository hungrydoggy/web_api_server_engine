  ...
  if (Array.isArray(obj) === true) {
    ...
    for (const key in obj) {
      if (key === 'include' && Array.isArray(obj[key]) === true) {
        ...
        const has_perm = db_util.hasFieldPermission(decoded, table_name, int_key, obj[key], permissions, [((is_write_mode)? '/write':'/read')]);
        if (!has_perm) {
          throw {
            statusCode: 403,
            body: {
  ...
  if (Array.isArray(obj) === true) {
    ...
    for (const key in obj) {
      ...
      if (ext_key[0] === '*' && ext_key[1] === '#') {
        const has_perm = db_util.hasFieldPermission(decoded, table_name, key, obj[key], permissions, ['/read']);
        if (has_perm) {
