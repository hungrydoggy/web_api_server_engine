...
function link (model, model_map) {
  ...
  for (const key in model.rawAttributes) {
    ...
    if (key[0] === '@' && key[1] === '#') {
      ...
      _linkHiddenField(model, model_map, key.substring(1), obj, 1, '@');
    }else if (key[0] === '@')
      _linkAssociation(model, model_map, key, obj);
    else if (key[0] === '#')
...
function linkByManyToMany (model, model_map, need_link_first=true, need_link_second=true) {
  ...
  for (const key in model.rawAttributes) {
    ...
    if (key[0] === '@' && key[1] === '#') {
      _linkAssociation(model, model_map, key, obj);
      _linkHiddenField(model, model_map, key.substring(1), obj, 1, '@');
    }else if (key[0] === '@' && m2m_fields.length < 2)
  ...
  if (need_link_first)
    _linkManyToMany(model, first [0], first [2], second[0], second[2], second[1], second[3]);
  if (need_link_second)
    _linkManyToMany(model, second[0], second[2], first [0], first [2], first [1], first [3]);
}
...
function _linkManyToMany (
  ...
  if (other_handler_options) {
    if (other_handler_options[0])
      options.onUpdate = _getHandleOptionFromChar(other_handler_options[0]);
    if (other_handler_options[1])
      options.onDelete = _getHandleOptionFromChar(other_handler_options[1]);
  }
  ...
  _assignInternalExternalField(model.getTableName(), m2m_as_name, m2m_as_name);
}
...
function _linkAssociation (model, target_models, key, obj) {
  ...
  if (has_many_name) {
    ...
    table_info.associationname_model_map[hasmany_as_name] = target_model;
    _assignInternalExternalField(model.getTableName(), hasmany_as_name, hasmany_as_name);
  }
...
function _linkHiddenField (model, model_map, key, obj, cmt_line_idx=0, field_prefix='') {
  
  const full_str = key.substr(1) + _getOptionStrFromComment(obj.comment, cmt_line_idx);
  ...
  _assignInternalExternalField(model.getTableName(), key, `${field_prefix}#${field_name}`);
}
