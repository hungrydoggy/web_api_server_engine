...
const model_dictionary = require('../../db/model_dictionary');
const db_util          = require('./db_util');
const jwt_util         = require('./jwt_util');

const archive_util =
  (process.env.ARCHIVE_DB === 'mongo')? require('./mongo_archive_util'):
...
module.exports = {
  ...
  async commonPost (
    ...
    for (const key of unique_keys)
      await _checkUniqueField(Table, undefined, key, params[key]);

    let new_row = (await Table.create(params, options)).toJSON();
    
    new_row = reviseResult(Table.getTableName(), new_row, permissions);
  ...
  async commonGet (
      decoded, Table, options, need_count=undefined) {
    ...
    items = reviseResult(Table.getTableName(), items, permissions);
  ...
  async commonGetById (decoded, Table, id, options, need_count=undefined) {

    if (decoded && decoded.options_from_permission) {
      const rows = await Table.findAll(decoded.options_from_permission);
      const has_perm = rows.filter((r) => parseInt(r.id) === parseInt(id)).length > 0;
      if (has_perm === false) {
        throw {
          statusCode: 403,
          body: {
    ...
    let item = res.toJSON();
    item = reviseResult(Table.getTableName(), item, permissions);
  ...
  async commonPut (decoded, Table, id, params, options, unique_keys = []) {
    ...
    for (const key of unique_keys)
      await _checkUniqueField(Table, id, key, params[key]);
  ...
  async commonDel (decoded, Table, id, options) {
  ...
  async throwIfNotExists (Table, id, options={}) {
    ...
    if (row === null) {
      throw {
        ...
        body: {
          message: `no ${Table.getTableName()} for id: ${id}`,
        },
      };
    }
  ...
  async loginTemplate (
    ...
    make_token_params_func,  /* (row) => #token_params */
    ...
    const token_params = make_token_params_func(row);
  ...
  if (Array.isArray(obj) === true) {
    ...
    for (const key in obj) {
      if (key === 'include' && Array.isArray(obj[key]) === true) {
        ...
        const int_key = db_util.toInternalFieldName(table_name, key);
        const has_perm = db_util.hasFieldPermission(table_name, int_key, permissions, [((is_write_mode)? '/write':'/read')]);
        if (!has_perm) {
          throw {
            statusCode: 403,
            body: {
              message: `no permission for writing ${key}`,
            },
          };
        }
        if (int_key !== key)
    ...
    if (obj[0] === '.') {
      ...
      let base = model_dictionary[words[0]];
      if (base === undefined)
        return obj;
...
function reviseResult (table_name, obj, permissions) {
  if (typeof permissions === 'string')
    permissions = JSON.parse(permissions);
  ...
  if (Array.isArray(obj) === true) {
    ...
    for (const value of obj)
      result.push(reviseResult(table_name, value, permissions));
    ...
    for (const key in obj) {
      ...
      if (ext_key[0] === '*') {
        ...
        result[ext_key] = reviseResult(model.getTableName(), obj[key], permissions);
      }else if (ext_key[0] === '#') {
        const has_perm = db_util.hasFieldPermission(table_name, key, permissions, ['/read']);
        if (has_perm)
          result[ext_key] = reviseResult(table_name, obj[key], permissions);
      }else {
        result[ext_key] = reviseResult(table_name, obj[key], permissions);
      }
    }
...
async function _checkUniqueField (Table, cur_id, field, value) {
  if (value === undefined)
    return;
  ...
  if (row !== null && row.id !== parseInt(cur_id)) {
    throw {
      statusCode: 400,
      headers,
      body: {
