...
module.exports = {
  makeCrud (
      ...
      option_map = {}, /*
                        ...
                        * option_map.postGetById(decoded, result)
                        * option_map.postGet    (decoded, result)
                        * option_map.postPut    (decoded, req_params, result)
...
async function _onPost (
  ...
  return route_util.template(event, permission, async (decoded) => {
    const essential_table_fields = Object.keys(Table.rawAttributes).filter((k) => {
      return k !== 'id' && k!=='createdAt' && k !== 'updatedAt' && Table.rawAttributes[k].allowNull === false;
    ...
    const result = await route_util.commonPost(
        decoded, Table, params, options, unique_keys, option_map.preRemoveExclude);
...
async function _onGetById (
  ...
  return route_util.template(event, permission, async (decoded) => {
    ...
    const result = await route_util.commonGetById(
      decoded, Table, event.params.id, options, need_count);

    if (option_map.postGetById)
      await option_map.postGetById(decoded, result);
...
async function _onGet (
  ...
  return route_util.template(event, permission, async (decoded) => {
    ...
    let {options, need_count} = route_util.checkRequestParams(
      event, [], ['options', 'need_count']);
    options = (options === undefined)? {}: JSON.parse(options);
    ...
    if (option_map.preGet)
      await option_map.preGet(decoded, options);

    if (option_map.onGet) {
      await option_map.onGet(decoded, Table, options);
    }

    const result = await route_util.commonGet(
      decoded, Table, options, need_count, option_map.preRemoveExclude);
...
async function _onPut (
  ...
  return route_util.template(event, permission, async (decoded) => {
    ...
    const result = await route_util.commonPut(decoded, Table, event.params.id, params, options, unique_keys);

    if (option_map.postPut)
      await option_map.postPut(decoded, event.params, result);
...
async function _onDel (
  ...
  return route_util.template(event, permission, async (decoded) => {
    ...
    if (table_fields.indexOf('#is_archived') >= 0) {
      ...
      result = await route_util.commonDel(decoded, Table, event.params.id, options);
